...
parser.add_argument('-l', '--legitimate', help='legitimate path', default='legitimate')
parser.add_argument('-m', '--malicious', help='malicious', default='malicious')
...
if __name__ == '__main__':
    ...
    # Launch legitimate
    for ffile in tqdm.tqdm(os.listdir(args.legitimate)):
        try:
            res = extract_infos(os.path.join(args.legitimate, ffile))
            res.append(1)
            ff.write(csv_delimiter.join(map(lambda x:str(x), res)) + "\n")
        except pefile.PEFormatError:
            print('\t -> Bad PE format')

    for ffile in tqdm.tqdm(os.listdir(args.malicious)):
        try:
            res = extract_infos(os.path.join(args.malicious, ffile))
            res.append(0)
